...
app.get('/*',express.static('public')); // important for path change
...
app.get('/auth/accepted',
	...
	function (req, res) {
      ...
      // res.sendFile('/user/index.html');
	    res.redirect('/userfile/index.html');
	});
...
function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    console.log("got profile name", firstname);
    done(null, userid); 
}
...
passport.serializeUser((userid, done) => {
    ...
    // console.log("SerializeUser. Firstname is",firstname);
...
passport.deserializeUser((userid, done) => {
    ...
    // console.log("deserializeUser. Fisrname is:", firstname);


    // here is a good place to look up user data in database using
    // dbRowID. Put whatever you want into an object. It ends up
    // as the property "user" of the "req" object. 
    let userData = {userId: userid.id, userName: userid.name.givenName};
    done(null, userData);
});


// Start of the OLD INDEX.JS
...
app.get('/all', async function(request, response, next) {
  ...
  console.log("server userid", request.user.userId)
  let results = await dbo.get_all(request.user.userId)
...
app.post(`/store`, async function(request, response, next) {
  ...
  console.log("Server recieved a post request at", request.body);

  console.log("what does the rquest contain", request.user);
  let datawithuserinfo = Object.assign({}, request.body, request.user);
  console.log(datawithuserinfo)  
  let activity = act.Activity(datawithuserinfo);
...
app.get('/reminder', async function(request, response, next) {
  ...
  let result = await dbo.get_most_recent_planned_activity_in_range(0, currTime, request.user.userId, request.user.userName)
...
app.get('/name', async function(request, response) {
  ...
  let realname = request.user.userName;
  console.log("index name: ", realname)
  response.send({username: realname});
});
